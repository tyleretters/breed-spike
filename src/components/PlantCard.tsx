...
export const PlantCard = ({ plant, selected, role, onClick, compact }: Props) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="mt-1 truncate font-mono text-[10px] text-[var(--color-dim)]"
        title={plant.genome}
      >
      ...
      {!compact && (
        <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-[var(--color-muted)]">
          ...
          {denovoFirsts > 0 && <span className="text-[var(--color-event-denovo)]">de novo ×{denovoFirsts}</span>}
          {retroCount > 0 && <span className="text-[var(--color-event-retro)]">retro ×{retroCount}</span>}
        </div>
      )}
    </button>
  )
